...
from src.services.smhi_service import _aggregate_values, _aggregate_thunder_as_probability


class TestAggregateValues:
    """Tests for the _aggregate_values function."""
    ...
    def test_aggregate_by_month(self) -> None:
        ...
        result = _aggregate_values(raw, Granularity.MONTH)
        assert len(result) == 1
    ...
    def test_aggregate_by_year(self) -> None:
        ...
        result = _aggregate_values(raw, Granularity.YEAR)
        assert len(result) == 1
    ...
    def test_aggregate_empty_input(self) -> None:
        ...
        result = _aggregate_values([], Granularity.MONTH)
        assert result == []
...
class TestAggregateThunder:
    ...
    def test_thunder_monthly_probability(self) -> None:
        ...
        result = _aggregate_thunder_as_probability(raw, Granularity.MONTH)
        assert len(result) == 1
        assert result[0].period == "2024-01"
        # 3/30 * 100 = 10%
    ...
    def test_thunder_empty(self) -> None:
        ...
        result = _aggregate_thunder_as_probability([], Granularity.MONTH)
        assert result == []
